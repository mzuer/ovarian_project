...
require(recount)
require(matrixStats)
# Rscript download_tcga_brca_lumAher2_recount2.R

startTime <- Sys.time()

cat(paste0("> START ", startTime, "\n"))

outFolder <- "DOWNLOAD_TCGA_BRCA_LUMAHER2_RECOUNT2"
...
###~~~ 5th FILTER: lumA her2 STATUS FILTER
...
cat(paste0("... filter 5 - lumA and her2: ", nLumA + nHer2 , "/", nrow(tcga_sampleAnnot), "\n"))

lum_samples <- c(lumA_samples, her2_samples)

tcga_sampleAnnot <- tcga_sampleAnnot[tcga_sampleAnnot$id_for_survival %in% c(names(lumA_samples), names(her2_samples)),]
stopifnot(nrow(tcga_sampleAnnot) > 0)
...
stopifnot(setequal(c(lumA_samples, her2_samples),lum_samples))
stopifnot(tcga_sampleAnnot$id_for_survival %in% names(lum_samples))
...
# stopifnot(length(her2_samps) == nHer2) not true here because dup samples
...
#*** update here
# ~~~~ KEEP ONLY THE ONES THAT PASSED THE ER STATUS FILTER
tcga_counts_raw_all <- tcga_counts_raw_all[,c(lumA_samps, her2_samps)]  # filter also filter 4
stopifnot(!is.na(tcga_counts_raw_all))  
cat(paste0(dim(tcga_counts_raw_all), "\n"))
...
tcga_sampleAnnot <- tcga_sampleAnnot[c(lumA_samps, her2_samps),]
...
tcga_counts_raw_all <- tcga_counts_raw_all[,c(lumA_samps, her2_samps)]
stopifnot(rownames(tcga_sampleAnnot) == colnames(tcga_counts_raw_all))
...
stopifnot(setequal(rownames(tcga_sampleAnnot), c(lumA_samps, her2_samps)))
stopifnot(lumA_samps %in% colnames(tcga_counts_all))
stopifnot(her2_samps %in% colnames(tcga_counts_all))
tcga_counts_all <- tcga_counts_all[,c(lumA_samps, her2_samps)]
tcga_sampleAnnot <- tcga_sampleAnnot[c(lumA_samps, her2_samps),]

stopifnot(colnames(tcga_counts_all) == c(lumA_samps, her2_samps))
stopifnot(ncol(tcga_counts_all) == nLumA + nHer2)
